...
def forward_h(lru_parameters, input_sequence):
    """
    Only computes the hidden state of the forward pass, for the case of mixing layers (LRU+MLP)
    """
    # All LRU parameters
    nu_log, theta_log, B_re, B_im, C_re, C_im, D, gamma_log = lru_parameters
    ...
    Bu_elements = jax.vmap(lambda u: B_norm @ u)(input_sequence)
    elements = (Lambda_elements, Bu_elements)
    _, h = parallel_scan(binary_operator_diag, elements)  # all x_k
